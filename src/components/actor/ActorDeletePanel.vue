...
import { ActorsAction } from '../../utils/actions/ActorsAction';
  ...
  const deleteMovie = async (): Promise<void> => {
    await fetch(ApiRoutesId.ACTORS + `/${actor.value.id}`, {
      headers: {
...
<template>
  <Panel :header="actor.firstName + ' ' + actor.lastName" style="background: #F5F5F5; padding: 10px 20px; border-radius: 8px; width: fit-content; margin: auto; width: 50svw; border: 1px solid #E4E4E4">
    <template #header>
        <strong class="font-bold">Voulez-vous vraiment supprimer "<i>{{ actor.firstName + ' ' + actor.lastName }}</i>"?</strong>
    </template>
